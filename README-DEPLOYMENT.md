...
| **1** | `1-validate-env.sh` | Validate .env variables + database CDC readiness | 2-3 min |
...
4. ✅ **Databases CDC-enabled** — run `db-prep/` prep scripts (fresh databases) or `./db-prep/generate-remediation.sh` (existing databases) to set up CDC infrastructure. Phase 1 validates all CDC prerequisites and blocks deployment if anything is missing
...
Use **INCLUDE_LIST** to explicitly specify which tables to replicate. This is safer than capturing all tables automatically.

```bash
# SQL Server: explicitly list CDC-enabled tables
SQLSERVER_TABLE_INCLUDE_LIST=dbo.customers,dbo.orders          # Capture ONLY these tables
SQLSERVER_TABLE_EXCLUDE_LIST=                                   # Leave blank when INCLUDE_LIST is set

# Aurora: explicitly list publication members to capture
AURORA_TABLE_INCLUDE_LIST=public.products,public.inventory     # Capture ONLY these tables
AURORA_TABLE_EXCLUDE_LIST=                                      # Leave blank when INCLUDE_LIST is set
```

**Why INCLUDE_LIST is safer:**
- **Explicit intent:** "Replicate ONLY these tables" is clear and auditable
- **New tables won't auto-replicate:** If you enable CDC on a new table in SQL Server or add a table to the publication, it won't automatically start replicating — requires conscious config update
- **Precedence:** If both INCLUDE and EXCLUDE are set, INCLUDE takes precedence

**Auto-Discovery (Default Behavior)**

If INCLUDE_LIST is blank, Debezium auto-discovers:
- **SQL Server:** all tables with CDC enabled in the database
- **Aurora:** all tables in the publication matching `AURORA_SCHEMA_INCLUDE_LIST`

⚠️ Auto-discovery is convenient but risky — new CDC-enabled tables will silently start replicating.

#### Sink-Level Table Filtering (Defense-in-Depth)

In addition to source-side filtering, configure sink TOPICS_REGEX to be explicit:

```bash
# Forward: SQL Server → Aurora
JDBC_SINK_AURORA_TOPICS_REGEX=sqlserver\\..*\\.dbo\\.(customers|orders)      # NOT .*\.dbo\..*

# Reverse: Aurora → SQL Server
JDBC_SINK_SQLSERVER_TOPICS_REGEX=aurora\\.public\\.(products|inventory)      # NOT \.public\..*
```

**Why explicit regex (not wildcards):**
- If source config drifts, sinks won't auto-subscribe to unexpected tables
- Easier to audit and debug (no silent auto-subscriptions)
- Matches INCLUDE_LIST philosophy

#### No-PK table handling

If any of your CDC tables lack a primary key, you **must** set `SQLSERVER_MESSAGE_KEY_COLUMNS` and/or `AURORA_MESSAGE_KEY_COLUMNS`. Without this, Debezium produces null-key records and JDBC sink connectors crash.
...
| `AURORA_SCHEMA_INCLUDE_LIST` | `public` | PostgreSQL schema to capture |
...
Validates .env variables, network connectivity, and database CDC readiness. All checks are blocking — fix all errors before proceeding. See [Phase 1 details](#phase-1-validate-env--database-cdc-readiness) above.

### Step 5: Set Up Databases for CDC (Before Deployment)

Before starting Phase 0, enable CDC on both source databases. Choose the approach that fits your situation:

**Existing databases with tables?** Use the remediation generator — it inspects your live databases and produces targeted, idempotent SQL scripts with only the missing CDC prerequisites:

```bash
./db-prep/generate-remediation.sh
# Review generated remediation-sqlserver.sql and remediation-aurora.sql, then apply
```

**Fresh databases?** Use the prep scripts. These scripts:
...
**SQL Server:**
```bash
# Generate a strong password for the cdc_reader database user
...
### Phase 1: Validate .env + Database CDC Readiness

```bash
./scripts/1-validate-env.sh
```

Validates (all blocking errors):

**Sections 1-5 — .env audit:**
- Every required variable has a value
- IP addresses match expected format, ports are numeric
- Regex conventions (capture groups, escaping, quoting)
- Dispatch mode configuration (SSM instance IDs or SSH key)

**Section 6 — Network connectivity:**
- SQL Server, Aurora, and Broker-1 reachable on expected ports

**Section 7 — Database CDC readiness** (requires `sqlcmd` and `psql`):
- SQL Server: auth, database exists, `db_owner` role, CDC enabled at database level, at least one CDC-enabled table, capture job exists
- Aurora: auth, `REPLICATION` privilege, `rds.logical_replication = on`, `wal_level = logical`, replication slot exists, publication exists with tables

**If Phase 1 fails on database checks**, use the remediation generator:
```bash
./db-prep/generate-remediation.sh    # Generates targeted fix scripts
# Review and apply the generated SQL, then re-run:
./scripts/1-validate-env.sh
```
...
**Kafka state only** (brokers stay running — use when you want to clean topics/connectors/schemas without restarting everything):

```bash
./scripts/teardown-reset-kafka.sh             # Deletes connectors, topics, consumer groups, SR subjects
./scripts/teardown-reset-kafka.sh --dry-run   # Preview what would be deleted
```

After running: `./scripts/6-deploy-connectors.sh` to redeploy.

**Full node reset** (destroys all containers, images, volumes — requires full redeploy from Phase 3):
...
| Phase 1: "CDC NOT enabled" | Database CDC not configured | Run `./db-prep/generate-remediation.sh`, review + apply generated SQL |
| Phase 1: "Replication slot not found" | Aurora not prepped for CDC | Run `./db-prep/generate-remediation.sh`, apply `remediation-aurora.sql` |
| Phase 1: "Publication has no tables" | No tables in Aurora publication | Add tables: `ALTER PUBLICATION cdc_publication ADD TABLE <table>` |
...
./scripts/ops-audit-cdc-enabled.sh --sqlserver    # Audit CDC-enabled tables
./scripts/teardown-reset-kafka.sh                 # Clean Kafka state (brokers stay up)
./scripts/teardown-reset-all-nodes.sh             # Full node reset
```
