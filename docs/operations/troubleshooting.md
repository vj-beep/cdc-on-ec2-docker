...
## Sink Connector Running but No Data Arriving (topics.regex Empty Partition Assignment)

### Symptoms

- Sink connector shows `RUNNING` status with all tasks running
- DLQ topic is empty (no errors)
- Source connector is producing records to Kafka topics
- Target database receives no data

### Root Cause

Sink connectors using `topics.regex` get partition assignments based on topics that **exist at consumer group join time**. If the sink starts before the source connector creates CDC topics, the consumer gets `Assignment(partitions=[])` and sits idle.

### Diagnosis

Check the Connect worker logs for the sink consumer's partition assignment:

```bash
docker logs <connect-container> 2>&1 | grep "jdbc-sink.*Assignment"
```

If you see `Assignment(partitions=[])`, the sink has no topics to consume from.

### Resolution

**Automatic (built-in):** Sink connectors include `consumer.override.metadata.max.age.ms=30000`. The consumer refreshes topic metadata every 30 seconds and will auto-discover new topics. Wait up to 30 seconds for self-healing.

**Manual restart:**
```bash
# Restart sink with all tasks (triggers fresh partition assignment)
curl -X POST http://localhost:8083/connectors/jdbc-sink-aurora/restart?includeTasks=true
curl -X POST http://localhost:8084/connectors/jdbc-sink-sqlserver/restart?includeTasks=true
```

**Verify fix:** After restart, check logs again — you should see partitions in the assignment:
```bash
docker logs <connect-container> 2>&1 | grep "Adding newly assigned partitions"
# Expected: Adding newly assigned partitions: sqlserver.pocdb.dbo.customers-0, ...
```

### Prevention

`6-deploy-connectors.sh` deploys source connectors before sinks with a 15-second gap, and runs a post-deploy verification that auto-restarts any sink with no running tasks.

If deploying connectors manually, always deploy sources first and verify CDC topics exist before deploying sinks.
