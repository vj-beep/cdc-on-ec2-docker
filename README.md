...
> ### 📌 Project Intent
>
> This repository is a **Reference Architecture** for building a **bi-directional Change Data Capture (CDC) pipeline** between SQL Server and Aurora PostgreSQL, orchestrated across **five AWS EC2 nodes** using Docker Compose and Confluent Platform.
>
> It delivers a **fully functional end-to-end flow** — including CDC capture, Kafka streaming, sink delivery, PII masking, and monitoring — using **three sample tables** as a working demonstration.
>
> **🚧 This is a foundational template, not a turnkey production solution.** It is designed to accelerate your evaluation and prototyping. Production deployments require customization of security, schemas, persistence, and scaling — see [Mandatory Customizations for Production](#mandatory-customizations-for-production) below.
>
> ### ⚠️ Disclaimer
>
> *This software is provided "as-is," without warranty of any kind, express or implied. The authors and contributors assume no liability for damages arising from its use. Users are solely responsible for all AWS costs incurred by the EC2 instances, databases, and networking resources, and for securing the infrastructure according to their organization's compliance and security standards.*
...
- [Mandatory Customizations for Production](#%EF%B8%8F-mandatory-customizations-for-production)
- [License](#license)
...
| **1** | `./scripts/1-validate-env.sh` | Validate .env + database CDC readiness | 2-3 min |
...
| **Audit** | `./scripts/ops-audit-cdc-enabled.sh` — Audit CDC-enabled tables on SQL Server or Aurora |
| **Remediation** | `./db-prep/generate-remediation.sh` — Generate targeted SQL fix scripts from live DB state |
| **Teardown** | `./scripts/teardown-reset-kafka.sh` — Clean Kafka topics, consumer groups, and schemas (brokers stay running) |
| | `./scripts/teardown-reset-all-nodes.sh` — Full node reset for re-deployment |
...
**This is customer responsibility** — must be done before Phase 2 (repo deployment). You have three options:

#### Option A: Use the prep scripts (fresh databases)

Run `db-prep/prep-sqlserver.sql` and `db-prep/prep-aurora.sql` — these set up CDC infrastructure, create the `cdc_reader` user, and include commented examples for creating tables and enabling CDC on them. See [README-DEPLOYMENT.md — Step 5](README-DEPLOYMENT.md#step-5-set-up-databases-for-cdc-before-deployment) for exact commands.

#### Option B: Use the remediation generator (existing databases)

If you already have databases with tables, run the remediation generator to produce targeted SQL scripts based on your current DB state:

```bash
./db-prep/generate-remediation.sh
```

This connects to both databases, inspects what's missing (CDC, replication slot, publication, user permissions), and generates idempotent fix scripts:
- `db-prep/remediation-sqlserver.sql` — enables CDC on your existing tables (detects PK vs no-PK)
- `db-prep/remediation-aurora.sql` — creates replication slot, publication with your tables, grants

Review the generated scripts, then apply:
```bash
source .env
...
#### Verify CDC readiness

Regardless of which option you used, Phase 1 validates all CDC prerequisites:

```bash
./scripts/1-validate-env.sh
```

Section 7 checks: SQL Server CDC enabled + tables + capture job + permissions; Aurora logical replication + wal_level + replication slot + publication + `cdc_reader` role + DML grants. All failures are blocking with exact remediation commands.
...
```bash
# Validate .env + database CDC readiness (blocking checks)
./scripts/1-validate-env.sh
```

Phase 1 validates everything: `.env` completeness, network connectivity, **and** database CDC readiness (SQL Server CDC status, Aurora replication slot/publication, user permissions). All checks must pass before proceeding.
...
```bash
./scripts/teardown-reset-kafka.sh          # Clean Kafka state (topics, groups, schemas) — brokers stay running
./scripts/teardown-reset-kafka.sh --dry-run # Preview what would be deleted
./scripts/teardown-reset-all-nodes.sh      # Full node reset (destroys everything, re-pull images)
```
...
## ⚠️ Mandatory Customizations for Production

Before deploying this in a business environment, the following updates are **required**:

* **🔐 Security & Networking:**
    * Replace default passwords in all `.env` and configuration files.
    * Restrict EC2 Security Groups to specific VPC CIDR blocks; do not leave ports 2181, 9092, or 8083 open to `0.0.0.0/0`.
    * Enable TLS encryption on all Kafka listeners and Connect REST endpoints.
    * Configure Confluent RBAC or Kafka ACLs for topic-level access control.
* **🗄️ Database Schema:**
    * The sample includes 3 boilerplate tables. You must update the Debezium connector JSON files to reflect your specific database names, schemas, and tables.
    * Review and adjust SMT transforms (PII masking, field mapping) for your data model.
* **💾 Data Persistence:**
    * The current configuration uses standard Docker volumes. For production workloads, ensure these are mapped to persistent **AWS EBS volumes** to prevent data loss during container lifecycle events.
    * Kafka broker data on NVMe is ephemeral — always maintain RF=3 and consider EBS-backed brokers for durability.
* **📈 Resource Scaling:**
    * The instance types and JVM heap settings in these Compose files are tuned for a "Sample/PoC" load. Scaling to high-throughput production data will require vertical scaling of EC2 instances and horizontal scaling of Kafka brokers.
    * Monitor consumer lag, Connect task throughput, and broker disk usage to right-size resources.
