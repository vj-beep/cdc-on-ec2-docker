...
if [[ "$DISPATCH_MODE" == "ssh" ]]; then
  ...
  NODE_IPS=("${BROKER_1_IP:-}" "${BROKER_2_IP:-}" "${BROKER_3_IP:-}" "${CONNECT_1_IP:-}" "${MONITOR_1_IP:-}")
fi
...
get_node_status() {
  ...
  local node_ip="${3:-}"  # IP address (SSM mode only, for display)

  if [[ -n "$node_ip" ]]; then
    echo -e "${BOLD}${BLUE}─ $node_name ($node_addr | $node_ip)${NC}"
  else
  ...
  if [[ "$DISPATCH_MODE" == "ssh" ]]; then
    output=$(ssh -n -i "${SSH_KEY_PATH}" -o StrictHostKeyChecking=no -o ConnectTimeout=5 \
      "${DEPLOY_USER}@${node_addr}" \
      "docker ps --filter label=com.docker.compose.project=cdc-on-ec2-docker --format json" 2>/dev/null || echo "")
    if [[ -z "$output" ]]; then
    ...
    command_id=$(aws ssm send-command \
      ...
      --parameters "commands=[\"docker ps --filter label=com.docker.compose.project=cdc-on-ec2-docker --format json\"]" \
  ...
  if [[ "$output" =~ \{.*\"State\" ]]; then
    ...
    while IFS= read -r line; do
      ...
      # Extract Name/Names and State from JSON object
      local name=$(echo "$line" | grep -oE '"(Name|Names)":"[^"]*' | head -1 | sed 's/"Name[s]*":"\(.*\)/\1/')
...
for i in "${!NODE_NAMES[@]}"; do
  get_node_status "${NODE_NAMES[$i]}" "${NODE_ADDRS[$i]}" "${NODE_IPS[$i]:-}"
done

echo -e "${BOLD}${BLUE}Legend:${NC}"
echo -e "  ${GREEN}●${NC} = Running | ${RED}●${NC} = Stopped | ${YELLOW}●${NC} = Unknown"
echo ""
CONNECT_IP="${CONNECT_1_IP:-<connect-ip>}"
MONITOR_IP="${MONITOR_1_IP:-<monitor-ip>}"

echo -e "${BOLD}${BLUE}─ Quick Reference (curl)${NC}"
echo -e "  ${GREY}Connect (fwd):  curl http://${CONNECT_IP}:8083/connectors?expand=status${NC}"
echo -e "  ${GREY}Connect (rev):  curl http://${CONNECT_IP}:8084/connectors?expand=status${NC}"
echo -e "  ${GREY}Schema Registry: curl http://${CONNECT_IP}:8081/subjects${NC}"
echo -e "  ${GREY}Control Center: http://${MONITOR_IP}:9021${NC}"
echo -e "  ${GREY}Grafana:        http://${MONITOR_IP}:3000${NC}"
echo -e "  ${GREY}Prometheus:     http://${MONITOR_IP}:9090${NC}"
echo -e "  ${GREY}ksqlDB:         http://${MONITOR_IP}:8088${NC}"
echo ""
